...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//дПереводитьЗапись
	//лПеревод = дОбщиеФункцииСервер.ПеревестиТекстЧерезЯндекс(Описание,, Ссылка);
	Если КоличествоСлов <= 370 И КоличествоСимволов <= 2500 Тогда //Иначе будет долго получать ответ не получит, ещё и зависнув его ожидая!
		//Хвала твиттеру!:)
		лПеревод = дОбщиеФункцииСервер.ПеревестиТекстЧерезRapidapi(Описание,,, Ссылка);
	КонецЕсли;
