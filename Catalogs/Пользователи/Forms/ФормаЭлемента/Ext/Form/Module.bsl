...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// ДЕНЬГИ
	ОбщегоНазначенияДневник.ИзменитьРазрешениеОткрыватьВнешниеОтчетыИОбработки(РазрешеноОткрытиеВнешнихОтчетовИОбработок, ПользовательИБИмя);
	// Конец ДЕНЬГИ
...
Процедура ОбщаяНастройкаФормы(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	Элементы.ПользовательИБРежимЗапуска.ТолькоПросмотр            = Истина;
	// Конец ДЕНЬГИ
...
Функция ОписаниеПользователяИБ(ДляПроверкиПервогоАдминистратора = Ложь)
	...
	// ДЕНЬГИ
	// Изменено условие для автоматического заполнения нужных прав. Вместо:
	//Если ПользователиСлужебный.ТребуетсяСоздатьПервогоАдминистратора(Результат) Тогда
	// добавлено:
	Если ПользователиСлужебный.ТребуетсяСоздатьПервогоАдминистратора(Результат) Или НЕ ЗначениеЗаполнено(Результат.Роли) Тогда
	// Конец ДЕНЬГИ
...
Функция НачальноеОписаниеПользователяИБ()
	...
	// ДЕНЬГИ
	РазрешеноОткрытиеВнешнихОтчетовИОбработок = ОбщегоНазначенияДневник.РазрешеноОткрытиеВнешнихОтчетовИОбработок(ПользовательИБИмя);
