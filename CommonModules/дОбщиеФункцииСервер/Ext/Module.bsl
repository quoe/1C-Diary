...
Функция ЧтениеXMLВДерево(Путь)
	...
	Исключение
		Сообщить("Не удалось обработать погодные данные. Ошибка: " + ОписаниеОшибки(), СтатусСообщения.Внимание);
		XMLФайл.Закрыть();
...
Функция ПеревестиТекстЧерезЯндекс(пТекст, пПеревод = "ru-en") Экспорт
