...
Процедура ОткрытьПроводник(ПутьККаталогуИлиФайлу) Экспорт
	...
	ТекстПредложения = НСтр("ru = 'Для открытия папки необходимо необходимо установить расширение работы с файлами.'");
...
// Не доступно в веб-клиенте. Для веб-клиента рекомендуется использовать метод платформы ПолучитьФайл.
//
// Параметры:
...
//
Процедура ОткрытьФайлВПрограммеПросмотра(ПутьКФайлу) Экспорт
	
#Если ВебКлиент Тогда
	
	ПоказатьПредупреждение(, НСтр("ru = 'Открытие файла в программе просмотра не доступно в веб-клиенте.'"));
...
#Иначе
	...
	Попытка
		...
		Если ФайлИнфо.Существует() Тогда
			...
			Если ОбщегоНазначенияСлужебныйКлиент.ЭтоРасширениеИсполняемогоФайла(ФайлИнфо.Расширение) Тогда 
				ПоказатьПредупреждение(, НСтр("ru = 'Запрещено открывать исполняемые файлы.'"));
				Возврат;
			КонецЕсли;
			
			ЗапуститьПриложение("file:///" + ПутьКФайлу);
...
//  ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("skype:call?echo123");
//
...
#КонецЕсли
	...
	ТекстПредложения = НСтр("ru = 'Для открытия ссылки необходимо необходимо установить расширение работы с файлами.'");
	ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Оповещение, ТекстПредложения, Ложь);
