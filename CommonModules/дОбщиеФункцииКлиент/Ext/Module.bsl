...
Функция ЧтениеJSON(пПутьКФайлу) Экспорт
	...
	Возврат дОбщиеФункцииКлиентСервер.ЧтениеJSON(пПутьКФайлу);
...
Процедура СоздатьСоСсылкойНаВыбраннуюЗапись(пСсылка, пПростаяФорма = Ложь) Экспорт
	
	СписокТекущиеДанныеСсылка 	= пСсылка;
	
	лФорма 			= "Документ.дДневник.Форма.ФормаДокумента";
	Если пПростаяФорма Тогда
		лФорма 		= "Документ.дДневник.Форма.ФормаДокументаПростая";
	КонецЕсли; 
	
	ФормаОбъекта    = ПолучитьФорму(лФорма);
