...
Процедура ЗаполнитьДеревоРазделовПоиска(ДеревоРазделовПоиска)
	...
	ПослеПолученияРазделовПолнотекстовогоПоискаРекурсивно(ДеревоРазделовПоиска);
...
&НаСервереБезКонтекста
Процедура ПослеПолученияРазделовПолнотекстовогоПоискаРекурсивно(ТекущаяСтрокаДерева)
	
	Если ТипЗнч(ТекущаяСтрокаДерева) = Тип("СтрокаДереваЗначений") Тогда 
		
		// Заполнение ПутьКДанным и ЭтоПодраздел выполняется после формирования дерева
	...
	Для каждого ПодчиненнаяСтрока Из ТекущаяСтрокаДерева.Строки Цикл 
		ПослеПолученияРазделовПолнотекстовогоПоискаРекурсивно(ПодчиненнаяСтрока);
...
//
&НаКлиентеНаСервереБезКонтекста
Процедура ПометитьВсеЭлементыДереваРекурсивно(ДеревоРазделовПоиска, ЗначениеПометки)
