...
Процедура HTMLТекстПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ЭтоНавигационнаяСсылка(СсылкаHTML.href) Тогда
		ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(СсылкаHTML.href);
	ИначеЕсли ЭтоСсылкаНаСправку(СсылкаHTML.href) Тогда 
		ОткрытьСправку(СсылкаHTML.href)
	Иначе 
...
Процедура ПриВыполненииПоискаНаСервере(КонтейнерСостояний, СтрокаПоиска, НаправлениеПоиска = "ПерваяЧасть")
	...
	ПараметрыПоиска = Обработки.ПолнотекстовыйПоискВДанных.ПараметрыПоиска();
	...
	РезультатПоиска = Обработки.ПолнотекстовыйПоискВДанных.ВыполнитьПолнотекстовыйПоиск(ПараметрыПоиска);
...
Функция НоваяHTMLСтраницаОшибки(КонтейнерСостояний)
	...
	ИначеЕсли КодОшибки = "СлишкомМногоРезультатов" Тогда
		...
		Если (Не ИскатьВРазделах)
			Или (ИскатьВРазделах И Не УказаныОбластиПоиска) Тогда 
...
Функция ЭтоНавигационнаяСсылка(Строка)
	...
	Возврат СтрНайти(Строка, "e1cib/data/") <> 0;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ЭтоСсылкаНаСправку(Строка)
